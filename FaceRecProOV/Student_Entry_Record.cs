    ...
    public partial class Student_Entry_Record : Form
    {
        ...
        public void GetData()
        {
            try
            {
                ...
                cc.con.Open();
                cc.cmd = new SqlCommand("SELECT RTRIM(Year) as [YEAR],RTRIM(Term) as [TERM],RTRIM(Subject) as [SUBJECT],RTRIM(Teacher) as [TEACHER],RTRIM(DateTime) as [DATE],RTRIM(Name) as [ROLL-NAME],RTRIM(Attendance) as [ATTENDANCE],Photo from Attendance order by name", cc.con);
        ...
        private void txtGuestName_TextChanged(object sender, EventArgs e)
        {
            try
            {
                cc.con = new SqlConnection(cs.DBConn);
                cc.con.Open();
                cc.cmd = new SqlCommand("SELECT RTRIM(Year) as [YEAR],RTRIM(Term) as [TERM],RTRIM(Subject) as [SUBJECT],RTRIM(Teacher) as [TEACHER],RTRIM(DateTime) as [DATE],RTRIM(Name) as [ROLL-NAME],RTRIM(Attendance) as [ATTENDANCE],Photo from Student WHERE name like '" + txtStudentName.Text + "%' order by name", cc.con);
                cc.da = new SqlDataAdapter(cc.cmd);
                cc.ds = new DataSet();
                cc.da.Fill(cc.ds, "Attendance");
                dgw.DataSource = cc.ds.Tables["Attendance"].DefaultView;
